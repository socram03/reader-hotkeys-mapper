...
const MAPPER_OVERLAY_ID = 'reader-hotkeys-mapper';
const TOAST_CONTAINER_ID = 'reader-hotkeys-toasts';
const FOCUS_STYLE_ID = 'reader-hotkeys-focus-style';
const FOCUS_CLASS = 'reader-hotkeys-focus';
...
const sites = {
	...
	olympus: {
		...
		getMainHref: () => {
			...
			if (match?.[1]) return `/series/${match[1]}`;
...
function setupRuntimeMessaging() {
	...
	chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
		if (!message?.type?.startsWith('reader:')) return false;
...
async function handleRuntimeMessage(message) {
	switch (message.type) {
		...
		case 'reader:toggle-focus':
			if (!runtime.site) return getReaderStatus();
			toggleFocusMode();
			return getReaderStatus();
...
function getReaderStatus() {
	...
	return {
		...
		lastReadIsCurrentChapter: Boolean(
			resumeTarget?.entry?.chapterHref
			&& currentChapterHref
			&& areComparableHrefsEqual(resumeTarget.entry.chapterHref, currentChapterHref)
		),
		settings: {
...
function getMappedSite(location) {
	...
	if (!mapping?.readingPrefix) return null;
...
function startAutoScroll() {
	if (runtime.autoScrollTimer) return;
	...
	runtime.autoScrollTimer = window.setInterval(() => {
		if (runtime.autoScrollPaused) return;
...
function refreshNavigationState() {
	...
	if (!nextHref || nextHref === runtime.nextHref) return;
...
function prefetchHref(href) {
	if (!href || href === runtime.prefetchedHref) return;
	...
	const runPrefetch = () => {
		fetch(href, { credentials: 'include' }).catch(() => undefined);
	};
...
function shouldRestoreResume() {
	...
	if (Date.now() - entry.updatedAt > RESUME_MAX_AGE_MS) return false;
	if (entry.scrollY < 180 && entry.percent < 0.05) return false;
	return true;
}
...
function checkAutoNext() {
	...
	if (!runtime.settings.autoNext || !runtime.nextHref || runtime.autoNextTimer) return;
...
function cancelAutoNext() {
	if (!runtime.autoNextTimer) return;
...
function toggleMapper() {
	if (closeMapper()) return;

	openMapper();
}

function openMapper() {
	if (runtime.mapper) return;
...
function getMatchingUserMapping(location, options: { includeDisabled?: boolean } = {}) {
	...
	return getHostUserMappings(location.host)
		...
		.filter(mapping => matchesPath(location.pathname, getAllReadingPrefixes(mapping)))
		.sort((left, right) => {
...
function getLongestMatchingPrefixLength(pathname, mapping) {
	return getAllReadingPrefixes(mapping)
		.filter(prefix => pathname.startsWith(prefix))
		.sort((left, right) => right.length - left.length)[0]?.length || 0;
}
...
function buildMappedActionDescriptor(element) {
	...
	if (!(clickable instanceof Element)) return null;
...
function buildSelectorCandidates(element) {
	...
	if (element.id) candidates.push(`#${escapeCssToken(element.id)}`);

	const name = element.getAttribute('name');
	if (name) candidates.push(`${element.tagName.toLowerCase()}[name="${escapeAttribute(name)}"]`);

	const title = element.getAttribute('title');
	if (title) candidates.push(`${element.tagName.toLowerCase()}[title="${escapeAttribute(title)}"]`);

	const ariaLabel = element.getAttribute('aria-label');
	if (ariaLabel) candidates.push(`${element.tagName.toLowerCase()}[aria-label="${escapeAttribute(ariaLabel)}"]`);
...
async function loadChapterMap(mainHref, overlay) {
	...
	searchInput?.addEventListener('input', event => {
		...
		const filtered = !query
			? chapters
			: chapters.filter(chapter => chapter.label.toLowerCase().includes(query) || chapter.href.toLowerCase().includes(query));
...
function getManhwaWebChapterHref(direction) {
	const chapterLink = findLink(link => {
		...
		if (!matchesPath(path, sites.manhwaweb.paths)) return false;
		return direction === 'siguiente'
			? text.includes('cap. siguiente') || text.includes('siguiente')
			: text.includes('cap. anterior') || text.includes('anterior');
	});
